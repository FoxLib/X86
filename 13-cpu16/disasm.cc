...
int X86::disas_modrm(int reg32, int mem32) {
...
int X86::disassemble(int address) {
...
int X86::debugout(int address = 0) {
    ...
    for (int i = 0; i < 25; i++) {
        ...
        int n = disassemble(address);
        fore = 0xFFFF00;
        sprintf(tmp, "%04X", address);
        print(tmp, 0, i*16);
        
        fore = ip == address ? 0xFFFFFF : 0xC0C0C0;
        sprintf(tmp, "%s", dis_row);
        print(tmp, 5*8, i*16);
        address += n;

        if (ip == address) match = 1;
    }

    // Регистры
    sprintf(tmp, "ax: %04X   sp: %04X", ax, sp); print(tmp, 50*8, 16*1);
    sprintf(tmp, "bx: %04X   bp: %04X", bx, bp); print(tmp, 50*8, 16*2);
    sprintf(tmp, "cx: %04X   si: %04X", cx, si); print(tmp, 50*8, 16*3);
    sprintf(tmp, "dx: %04X   di: %04X", dx, di); print(tmp, 50*8, 16*4);
    sprintf(tmp, "ip: %04X", ip);                print(tmp, 50*8, 16*5);

    // Флуги
    sprintf(tmp, "%c%c%c%c%c%c%c%c%c",
        flags & 0x800 ? 'O' : ' ',
        flags & 0x400 ? 'D' : ' ',
        flags & 0x200 ? 'I' : ' ',
        flags & 0x100 ? 'T' : ' ',
        ...
        flags & 0x10  ? 'A' : ' ',
        flags & 0x04  ? 'P' : ' ',
        flags & 0x01  ? 'C' : ' '); print(tmp, 50*8, 16*6);
